...
async function cargarTorneos() {
    ...
    torneos.forEach(t => {
        ...
        html += `<tr><td>${t.fecha_evento || 'Sin fecha'}</td><td><strong>${t.franquicia || '-'}</strong></td><td>${t.nombre}</td><td style="color: ${colorEstado}; font-weight: bold;">${t.estado}</td><td><button class="btn-ver" onclick="verTorneo(${t.id}, '${t.nombre}')">👁️ Ver</button> <button class="btn-borrar" onclick="eliminarTorneo(${t.id}, '${t.nombre}')">🗑️ Borrar</button></td></tr>`;
    });
...
async function repararEloGlobal() {
    if (!confirm("🔄 ¿Iniciar Recálculo Global?")) return;
    ...
    msg.innerText = "⏳ 2. Simulando y reparando Libro Mayor...";
    ...
    msg.innerText = "⏳ 3. Guardando puntos finales...";
...
async function verTorneo(idTorneo, nombre) {
    document.getElementById('panelLista').style.display = 'none'; document.getElementById('panelDetalle').style.display = 'block';
    ...
    let htmlVisual = '';
    batallas.forEach(b => {
        if(b.resultado === 'bono') return; // Los bonos no se muestran en el visualizador 1vs1
