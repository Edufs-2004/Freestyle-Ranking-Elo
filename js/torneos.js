...
function filtrarBuscador() {
    ...
    if (resultados.length === 0) {
        html += `<div class="sugerencia-item" style="color: #888;">No se encontraron competidores...</div>`;
    } else {
    ...
    let coincidenciaExacta = mcsDisponibles.find(mc => {
        let akaSafe = mc.aka ? String(mc.aka).toLowerCase() : "";
...
function agregarChip(id) {
    ...
    if (mc) mcsSeleccionados.push(mc);
...
async function crearYAgregarMC(nombre) {
    ...
    document.getElementById('sugerenciasMCs').innerHTML = '<div class="sugerencia-item" style="color:var(--neon-cyan);">⏳ Creando e insertando en BD...</div>';
    
    const { data, error } = await supabase.from('competidores').insert([{ 
        aka: nombre, 
        nacionalidad: '🌍', 
        foto: 'https://via.placeholder.com/150/373752/FFFFFF?text=MC', 
        elo_actual: 1500, 
        batallas_totales: 0 
    }]).select();
    
    if (error || !data || data.length === 0) {
        alert("Error al crear el competidor. Refresca la página.");
